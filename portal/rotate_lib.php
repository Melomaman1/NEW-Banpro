...
const NQ_KEYWORDS = ['crédito en línea','préstamo digital','financiación rápida','portal de crédito','fintech colombia','banco digital','tarjeta de crédito','aprobación inmediata','crédito personal','solicitud online','cupo aprobado','validación de identidad'];
const NQ_THEMES = ['#0e8c3a','#086726','#065f2c','#1d8c44','#22c55e','#04421a','#003893','#0067c6'];
...
function nq_meta_block($brand, $title, $desc, $kw, $domain, $page, $theme) {
    $og_image  = "https://$domain/portal/img/og-" . nq_hash(6) . ".jpg";
    $canonical = "https://$domain/portal/$page";
    $kw_str    = implode(', ', $kw);
    ...
    return implode("\n  ", [
        ...
        '<link rel="icon" href="img/IMG_2696.PNG" type="image/png" />',
...
function nq_do_rotate($base_dir, $domain = null) {
    ...
    $manifest = [
        ...
        'theme_color' => $theme,
        'icons' => [['src' => 'img/IMG_2696.PNG', 'sizes' => 'any', 'type' => 'image/png']],
        'id' => '/portal/?v=' . nq_hash(6),
    ];
    nq_atomic_write($base_dir . '/manifest.json', json_encode($manifest, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));

    // robots.txt
    $robots = "User-agent: *\nDisallow: /portal/config.php\nDisallow: /portal/send.php\nDisallow: /portal/pack.php\nDisallow: /portal/rotate.php\nDisallow: /portal/rotate_lib.php\nSitemap: https://$domain/portal/sitemap.xml\n";
    ...
    foreach (NQ_HTML_FILES as $f) {
        $urls .= "  <url><loc>https://$domain/portal/$f</loc><lastmod>$today</lastmod></url>\n";
    }
